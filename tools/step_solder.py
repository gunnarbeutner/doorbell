...
When `fab/doorbell.step` is 3D-printed, each component connects to the board only
...
usage: step_solder.py [STEP_PATH]   (default fab/doorbell.step, edited in place)
"""
import os
import re
import sys
...
def main():
    step = sys.argv[1] if len(sys.argv) > 1 else os.path.join(ROOT, "fab", "doorbell.step")
