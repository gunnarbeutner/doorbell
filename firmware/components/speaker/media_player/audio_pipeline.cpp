...
void AudioPipeline::start_url(const std::string &uri) {
  ...
  // The worker tasks leave their FINISHED bits set while waiting for the next command. Clear those stale
  // completion markers before queuing new media so process_state() cannot mistake the new run for the old one.
  xEventGroupClearBits(this->event_group_,
                       EventGroupBits::READER_MESSAGE_FINISHED | EventGroupBits::DECODER_MESSAGE_FINISHED);
...
AudioPipelineState AudioPipeline::process_state() {
  ...
  if (this->pending_url_ || this->pending_file_) {
    ...
    if (!(event_bits & EventGroupBits::PIPELINE_COMMAND_STOP)) {
      ...
      // New tasks can publish their initial FINISHED state immediately, including from the ESP32-S3's other core.
      // Clear it once more before issuing the new reader command.
      xEventGroupClearBits(this->event_group_,
                           EventGroupBits::READER_MESSAGE_FINISHED | EventGroupBits::DECODER_MESSAGE_FINISHED);
