...
esp_err_t I2SAudioSpeaker::start_i2s_driver(audio::AudioStreamInfo &audio_stream_info) {
  ...
  const audio::AudioStreamInfo output_stream_info(output_bits_per_sample, audio_stream_info.get_channels(),
  ...
  if (this->tx_handle_ != nullptr) {
    if (this->current_stream_info_ == audio_stream_info && this->output_stream_info_ == output_stream_info) {
      return ESP_OK;
    }
