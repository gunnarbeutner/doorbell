...
Per channel it writes:
  runs/<out>/ch<n>.csv   full true voltages (time_s, volt), for analysis
  runs/<out>/ch<n>.wav   16-bit PCM of the AC content (DC pedestal removed, peak-normalized),
                        the FULL record by default; pass --skip-silence to gate flat stretches.
Plus, per recording:
  runs/<out>/preview.png  DC baseline + AC component per channel, on a shared time axis (unless --no-plot).
  runs/<out>/meta.json    capture timestamp + acquisition params (the date is no longer in the filename, so
...
def process_capture(d, chans, run_dir, args):
    ...
    for i, ch in enumerate(chans):
        ...
        if not (mask.any() or vpp >= args.min_vpp):
            ...
            continue
        write_csv(os.path.join(run_dir, f"ch{ch}.csv"), x, volts)
        audio = ac[mask] if (args.skip_silence and mask.any()) else ac
        rate = write_wav(os.path.join(run_dir, f"ch{ch}.wav"), audio, fs)
        written += 1
    ...
    if args.plot and plot_data:
        if write_plot(os.path.join(run_dir, "preview.png"), x, plot_data, args.hp_ms):
            print(f"  wrote preview.png ({len(plot_data)} channel(s): DC + AC)")
    return written
...
def main():
    ...
    ap.add_argument("--out", default="capture",
                    help="run name -> runs/<out>/ch<n>.csv/.wav (+ runs/<out>/meta.json); pick a unique name per capture")
    ...
    with PYDHO800(address=args.host, port=5555, useNumpy=True, rawMode=True) as dho:
        ...
        run_dir = os.path.join(RUNS_DIR, args.out)
        os.makedirs(run_dir, exist_ok=True)
        if process_capture(d, chans, run_dir, args):
            write_meta(os.path.join(run_dir, "meta.json"), now, chans, args.probe, d)
            print(f"  wrote meta.json (captured_at {now:%Y-%m-%dT%H:%M:%S} + acquisition params)")
        else:
