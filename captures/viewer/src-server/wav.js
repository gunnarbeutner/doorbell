...
export async function wavInfo(capturesDir, basename, ch) {
  let fh;
  try {
    fh = await open(wavPath(capturesDir, basename, ch), 'r');
...
export async function serveWav(capturesDir, basename, ch, req, res) {
  const path = wavPath(capturesDir, basename, ch);
  let size;
  try {
