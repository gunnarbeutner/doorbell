// Dev server for the captures recording viewer.
//   /api/recordings          list recordings in captures/runs/ (scan only)
...
//   /api/note?rec=           the recording's notes.md analysis note
//   /preview/<base>.png      the legacy overview PNG
...
const CAPTURES_DIR = process.env.CAPTURES_DIR ? process.env.CAPTURES_DIR : join(ROOT, '..');
...
async function recordingChannels(rec) {
  ...
  const all = await listRecordings(CAPTURES_DIR);
...
async function handleMeta(rec, res) {
  ...
  for (const ch of info.channels) {
    const c = await loadChannel(CAPTURES_DIR, CACHE_DIR, rec, ch);
    const wav = await wavInfo(CAPTURES_DIR, rec, ch);
...
async function handleNote(rec, res) {
  ...
  try {
    const md = await readFile(notePath(CAPTURES_DIR, rec), 'utf8');
...
async function handlePng(file, res) {
  ...
  try {
    const data = await readFile(pngPath(CAPTURES_DIR, base));
...
createServer(async (req, res) => {
  ...
  try {
    ...
    if (path === '/api/recordings') return sendJson(res, { recordings: await listRecordings(CAPTURES_DIR) });
    ...
    if (path === '/api/wav') {
      ...
      return await serveWav(CAPTURES_DIR, rec, ch, req, res);
    }
    ...
    if (path.startsWith('/preview/')) return await handlePng(path.slice('/preview/'.length), res);
    return await serveStatic(path, res);
  } catch (e) {
    fail(res, path.startsWith('/api/') ? 500 : 404, String(e?.message || e));
  }
}).listen(PORT, () => {
  console.log(`captures viewer → http://localhost:${PORT}   (data: ${CAPTURES_DIR})`);
});
