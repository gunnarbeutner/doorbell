# captures viewer — interactive scope-recording analysis UI

A local web app for the `captures/runs/` recordings: play the audio, zoom into events, read true
voltages, measure with cursors, and see the gong tones — a live alternative to the static
`preview.png` overviews.
...
on v26). Override the port with `PORT=…`, or the data dir with `CAPTURES_DIR=…` (defaults to `../`).

## What it does

- **Recording dropdown** — every recording in `captures/runs/` (one subdir per recording, newest first).
...
- **AC/DC** toggle (`d`), the recording's `notes.md` note (rendered), and the legacy `preview.png` for
  reference.
