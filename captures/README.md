# captures — Rigol DHO804 capture tooling
...
For an interactive alternative to the static `preview.png` overviews — play the audio, zoom into events,
...
Each run writes `runs/<out>/ch<n>.{csv,wav}` per channel, plus one overview image `runs/<out>/preview.png`
(**DC baseline + AC component per channel**, shared time axis — use `--no-plot` to skip) and a
`runs/<out>/meta.json` sidecar (capture timestamp + acquisition params), and prints sample count, rate, Vpp,
...
> `our-ring-door-open`, `door-open-standalone`). The capture date lives in `meta.json`
...
> ~4–6× smaller) rather than the raw `.csv`. Compress before committing (`cd runs/<name> && zstd --ultra -22 *.csv`)
> and read back with `zstd -dc file.csv.zst` (or `zstdcat`). The `.wav`, `preview.png`, `meta.json`, and
> per-session `notes.md` are committed uncompressed.
