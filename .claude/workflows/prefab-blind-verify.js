...
const mkBlind = (repo) =>
  [
    ...
    `- Allowed ground truth ONLY: the netlist at /tmp/prefab/doorbell.net, the schematic ${repo}/kicad/doorbell.kicad_sch (connectivity + component values + MPN/LCSC only), the datasheets in ${repo}/docs/, and the MEASURED bus voltages in ${repo}/captures/runs/ (real-world data, not design claims).`,
    '- The captures are STIMULI, not just an envelope: when a contact/path is closed in some state, the measured waveforms the matching captures/runs/ scenario shows on the connected lines are the signals that traverse it — use their amplitudes/spectra (per the runs’ notes.md) in transfer calculations, never an idealized quiet line.',
  ...
  properties: {
    ...
    sources: {
      ...
      items: {
        ...
        additionalProperties: false,
        properties: {
          file: { type: 'string', description: 'exact path under docs/, e.g. docs/datasheets/GAQY212GS_datasheet.pdf' },
...
const BLOCKS = [
  ...
  { key: 'absmax', title: 'Abs-max sweep on every IC pin', high: true,
    q: 'For every pin of every active IC on the board, given the measured bus voltages in captures/runs/ and the connectivity, does any pin exceed its datasheet absolute-maximum in normal operation OR under a single bus-line fault/miswire? List each at-risk pin with computed vs rated voltage.' },
  { key: 'rx', title: 'RX audio front-end vs codec abs-max', high: true,
    q: 'Trace the bus listen pair (lines 2 and 1) to the audio codec microphone/analog input pins. Using the measured gong on line 2 from captures/runs/, compute the AC swing and DC level at those input pins and compare to the codec mic-input absolute-max and operating range. Does the on-board network keep them in range on every ring?' },
  ...
  // CROSS-COUPLING & COMPOSED STATES: every block above verifies a path DOES its job, mostly in one
  // state at a time — none verifies a path carries ONLY its job across composed states (bus scenario
  // × actuator states). That gap let a real bug ship: with the talk gate closed during a session, the
  // measured gong AC riding the shared listen line traversed the (correct, in-spec) handshake bridge
  // onto the talk line, tens of dB above the intended transmit level. These two blocks own that class.
  { key: 'xtalk', title: 'Unintended-signal sweep (cross-coupling through engaged paths)', high: true,
    q: 'Build two inventories first: (1) SWITCHING ELEMENTS — every SSR/relay/mechanical-switch contact on the board, plus any bus-energised latch, each with its idle state (NO/NC) and what closes it (contacts sharing a drive net close together; a bus-energised latch closes whenever the bus scenario energises it, no MCU involved). (2) LIVE SOURCES — for each measured scenario in captures/runs/ (idle, own ring with its gong, held session, door-open, neighbour ring, floor call), every AC or DC signal the capture notes show on any bus line, PLUS board-side sources (the codec DAC output) and any electro-acoustic transducer that generates a voltage from room sound. Then sweep the composed states: for every reachable combination of closed contacts (idle-closed NC contacts and the bus-driven latch included) under each scenario, trace every galvanic path from every live source to every bus line and every codec/analog input, and compute the arriving level from the divider/filter math using the measured amplitudes/spectra. Compare each arriving UNINTENDED level against the INTENDED signal level at that node in that state (derive the intended level from topology — e.g. what the transmit chain is sized to deliver, what the receive tap is sized to accept). Prune only combinations that close no additional path. Flag every node where an unintended contributor lands less than 20 dB below the intended signal — or where an unintended DC shifts an operating point — with the full path, the state combination that produces it, and the numbers.' },
  { key: 'transients', title: 'State-transition transients on shared lines', high: true,
    q: 'For every state TRANSITION the board can make — each SSR/relay/switch contact making or breaking into its surrounding network, and each board-side source turning on or off (an amplifier/DAC output stage enabling into an AC-coupling capacitor, a gate drive asserting) — derive the transient delivered to every bus line and every codec/analog pin: the pre-transition charge state of each coupling/filter capacitor in the path, the step size at the switching instant, the divider to each victim node, and the settling time constant. Use the measured line levels in captures/runs/ as the rails behind each contact. Flag (a) any make/break that delivers a step to a shared bus line or analog pin that is not small against the intended signal there, (b) any transition whose safety depends on an ORDERING of two contacts that no circuit element enforces, and (c) any enable/disable transient the datasheet itself warns about (pop/click, soft-start) that the surrounding network does not absorb.' },
]
...
const gate = await agent(
  `You are the PRE-FAB GATE. Compare each blind finding (and its adversarial refutation) against the design’s own CLAIMS, weigh the verification-coverage audit, and decide whether the board is safe to fabricate.\n\n` +
    ...
    `SIGNAL CONTAMINATION IS FUNCTIONAL: a finding that an unintended contributor arrives within ~20 dB of the intended signal at a functional node (or that a state transition thumps a shared line) is a real bug even though every part is connected, in-polarity, and inside abs-max — never downgrade it for passing the ratings checks. ` +
